...
| **file .tgz (offline)** | Môi trường không ra được npm registry | `vdf-webview-miniapp-sdk-2.1.0.tgz` |
...
**Bước 1:** Lấy file `vdf-webview-miniapp-sdk-2.1.0.tgz` (sinh bằng `npm pack` từ `packages/core`) và copy vào thư mục `core-lib/` trong project
```bash
mkdir -p core-lib
cp vdf-webview-miniapp-sdk-2.1.0.tgz core-lib/
```
...
{
  "dependencies": {
    "vdf-webview-miniapp-sdk": "file:core-lib/vdf-webview-miniapp-sdk-2.1.0.tgz"
  }
}
```
